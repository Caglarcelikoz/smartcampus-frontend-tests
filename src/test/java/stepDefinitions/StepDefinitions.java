...
public class StepDefinitions {
    ...
    WebElement foundCompanyMessage;
    ...
    final String QUESTION_MESSAGE_COMPANY = "Which company are you looking for?";
    ...
    public void iClickOnTheCompaniesButtonInTheCUI() {
        companiesButtonCUI.click();
    }
    ...
    public void iVerifyThatThereTheCUIShowsAMessageWithTheTextCompany() {
        waiting();
        categoryMessageCUI = webDriver.findElement(By.xpath("/html/body/div[1]/div[2]/div[1]/div/div/div[3]/div/div/p"));
        Assert.assertEquals("companies", categoryMessageCUI.getText());

        if (!categoryMessageCUI.getText().equals("companies")) {
            error = 1;
        }
    }

    @Then("^I Verify That The CUI Asks Which Company The User Is Looking For$")
    public void iVerifyThatTheCUIAsksWhichCompanyTheUserIsLookingFor() {
        waiting();
        questionMessage = webDriver.findElement(By.xpath("/html/body/div[1]/div[2]/div[1]/div/div/div[4]/div/div[2]/div/p"));
        Assert.assertEquals(QUESTION_MESSAGE_COMPANY, questionMessage.getText());
        if (!QUESTION_MESSAGE_COMPANY.equals(questionMessage.getText())) {
            error = 1;
        }
    }

    @Then("^I Verify That The CUI Shows The Correct Messages When The Given Company Name Exists$")
    public void iVerifyThatTheCUIShowsTheCorrectMessagesWhenTheGivenCompanyNameExists() {
        waiting();
        foundCompanyMessage = webDriver.findElement(By.xpath("/html/body/div[1]/div[2]/div[1]/div/div/div[6]/div/div[2]/div[1]/p"));
        Assert.assertTrue(foundCompanyMessage.getText().contains("I found one company named"));
        if (!foundCompanyMessage.getText().contains("I found one company named")) {
            error = 1;
        }
    }

    @When("^I Click On No Button In Company Flow$")
    public void iClickOnNoButtonInCompanyFlow() {
        waiting();
        noButtonBuildings = webDriver.findElement(By.xpath("/html/body/div[1]/div[2]/div[1]/div/div/div[7]/div/div[2]/div[2]/button"));
        noButtonBuildings.click();
    }
    ...
    public void iVerifyThatTheCUIShowsACorrectFinalMessageInTheCompanyFlow() {
        waiting();
        finalMessageCompanies = webDriver.findElement(By.xpath("/html/body/div[1]/div[2]/div[1]/div/div/div[9]/div/div[2]/div/p"));
        Assert.assertEquals(FINAL_MESSAGE, finalMessageCompanies.getText());
        if (!FINAL_MESSAGE.equals(finalMessageCompanies.getText())) {
            error = 0;
        }
    }

    @When("^I give in CompanyName (.*) On The InputField$")
    public void iGiveInCompanyNameCompanyNameOnTheInputField(String companyName) {
        inputFieldCUI = webDriver.findElement(By.id("inputField"));
        inputFieldCUI.sendKeys(companyName);
    ...
    public void iVerifyThatTheInputfieldIsDisabled() {
        Assert.assertFalse(webDriver.findElement(By.id("inputField")).isEnabled());
